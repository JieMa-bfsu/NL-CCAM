...
def returnCAM_(feature_conv, weight_softmax, class_idx, reverse_idx, h_x, j, threshold, thr, function=None):
    ...
    cam1 = weight_softmax[reverse_idx.cpu().numpy()].dot(feature_conv.reshape((nc, h * w)))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", default='model/vgg_cnn_cub_100.pth', type=str)
            ...
            for i, (img_path, image_tensor, image_level) in enumerate(test_loader):
                ...
                for j in range(5):
                    ...
                    if j == 0:
                        ...
                        cv2.rectangle(img, (int(bounding_box_gt1[0]), int(bounding_box_gt1[1])), (int(bounding_box_gt1[2]), int(bounding_box_gt1[3])), (0,255,0), 4)
                        cv2.rectangle(img, (int(bounding_box[0]), int(bounding_box[1])), (int(bounding_box[2]), int(bounding_box[3])), (255,0,0), 4)
                        cv2.imwrite('./result/'+img_path[0].split('/')[-1], img)
